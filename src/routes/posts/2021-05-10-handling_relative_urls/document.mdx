...
Much like any untrusted user input, relative URLs should be canonicalised, sanitised, and then validated - in that order. Canonicalisation and sanitation should be done through established URL parsing libraries such as [URL Node package](https://nodejs.org/dist/latest-v8.x/docs/api/url.html#url_the_whatwg_url_api) that follow the [WHATWG standard](https://url.spec.whatwg.org/). The output of these operations should then be validated using a strict pattern, only allowing required characters. Dangerous characters such as `@`, `#` and multiple `/` characters should not be on the allow list.
...
# How Do We Handle Relative URLs?
...
## References:
- [Demo project](https://github.com/arbitraryrw/url-parsing)
