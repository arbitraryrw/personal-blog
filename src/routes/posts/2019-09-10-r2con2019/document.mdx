...
The encode function breakpoint at `0x0048a32f` targets the instruction `xor r10d, edx`. Notably, the instruction above `0x0048a32f` at `0x0048a32b` appears to be referencing an index of `RSI`. This means that the value stored in `RDX` is not static and is an offset to `RSI`, meaning it will change with each loop (unless the next offset contains the same value).
