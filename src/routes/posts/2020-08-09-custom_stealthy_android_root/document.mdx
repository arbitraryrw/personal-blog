...
On the other hand, working on a non-rooted device is not a straightforward process either. Although there are methods to use bread and butter tools on non-rooted devices, such as [Frida](https://frida.re/), generally it is more cumbersome. From my experience, it often adds additional complexity and can introduce more problems than it solves. Especially when working with multiple applications. A great example of this is: to be able to use [Frida Gadget](https://frida.re/docs/gadget/) you need to repackage the application, however, by repacking the application you now need to bypass APK signature detections.

The way that I usually handle this dilemma is by using a rooted device and bypassing any root detection controls that get in the way. Fortunately, there are many tools that offer effective root bypasses that work on most applications, such as [Objection](https://github.com/sensepost/objection) and [Magisk Hide](https://www.xda-developers.com/how-to-use-magisk/). Unfortunately, these tools are not always effective, many sophisticated applications are not so easily defeated. When this happens you’re forced to manually reverse the application and write a custom hook / patch it statically. Depending on the obfuscation / tamper resistance of the application this can be extremely time consuming. This is pretty much the worst case scenario.

The purpose of this post is to present an alternative approach that bypasses most root detection methods. The aim of this is to help prevent the worst case scenario and improve your quality of life when it comes to testing hardened applications.
...
Browse to the directory and install the required python dependencies. I would recommend using a [virtual envionrment](https://docs.python.org/3/tutorial/venv.html). In addition, install [Radare2](https://rada.re/n/) as the `r2-pipe` python package depends on this system utility.
...
The next step is to flash the patched SuperSU archive onto the test device. Connect the test device to the host machine set up with the required tools / utilities. Verify the device is visible by running `adb devices`. If it is visible, then execute the following command to boot the device to the bootloader.
...
This will then begin the installation, you should see the installation logs below.

<img src={twrpInstallLogs} className={styles["screenshot"]} alt="TWRP installation logs" />

Finally, press the `Reboot System` button. I would recommend not installing the TWRP app for detection purposes. This introduces another unnecessary signature and defeats the purpose of this exercise.
...
At this point the patched root is working and you’re now able to access the root user using the patched `nu` utility, instead of the conventional `su` utility. However, there are still a few loose ends that need to be cleaned up. For example, the SuperSU management application is still present, this is an extremely common technique for root detection. I would highly recommend removing the management application because of ths. In addition, the management application is not even aware of the root so it serves no purpose.

<img src={superSuNotDetected} className={styles["screenshot"]} alt="SuperSu management app does not detect root" />

This setup as it currently stands will be quite effective, however, it will fail the [SafetyNet](https://developer.android.com/training/safetynet/attestation#use-response-server) API checks, for context: 

_“The SafetyNet Attestation API is an anti-abuse API that allows app developers to assess the Android device their app is running on” - [Android Docs](https://developer.android.com/training/safetynet/attestation#:~:text=The%20SafetyNet%20Attestation%20API%20is,on%20a%20genuine%20Android%20device.)._ 
...
You can either use one of these kernels or compile your own custom kernel as I have [here](https://github.com/arbitraryrw/android_kernel_oneplus_msm8998). This can then be flashed onto the device the exact same way the patched SuperSU archive was earlier in this post. Once this has been done, the device should pass the SafetyNet checks (tested using the [SafetyNet Test](https://play.google.com/store/apps/details?id=org.freeandroidtools.safetynettest&hl=en_GB) app) as seen below.

<img src={safetynetPassImage} className={styles["screenshot"]} alt="SafetyNet basic and CTS pass" />

# Conclusion 

The approach presented in this post should help you create an effective test device that allows you to focus on what is really important. Instead of wasting precious time bypassing root detections and unblocking tools, you can invest time into looking at interesting problems. For more information on setting up your device for security testing, I would recommend reading the [MSTG - testing setup section](https://mobile-security.gitbook.io/mobile-security-testing-guide/android-testing-guide/0x05b-basic-security_testing#basic-android-testing-setup).
