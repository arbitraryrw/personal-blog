...
As the description suggests, it is a 12-pin digital lock that requires 12 separate keys to unlock the flag. The challenge is an unobfuscated stripped x86 binary that requires some creative problem solving. This post documents the journey of solving this challenge, mistakes and all.
...
### Initial Analysis
After downloading the challenge, the first step is to run the binary. Unfortunately in this case the challenge is rate limited according to the error message below. Apparently the binary can only be unlocked once per hour at 3 minutes past the hour. The next step was to first bypass this _feature_ to speed up the process.
...
If it is three minutes past the hour, it opens and reads from the file `/tmp/masterkey.in` at offset `0x004018fa` using `fopen`. The result of this function is then stored in the `rax` register and then validated at offset `0x00401908`, docs for `fopen` can be found [here](https://man7.org/linux/man-pages/man3/fopen.3.html).
...
Unforunately, this caused a segmentation fault. I jumped the gun a bit there so I went back to figuring out what was actually going on in more detail.
...
With that in mind, it looks like this functional call can be patched out completely statically. I replaced the function with an instruction to populate the `eax` ([accumulator](https://en.wikibooks.org/wiki/X86_Assembly/X86_Architecture)) register with the expected value. I seek to the offset of the function at `0x004018c`, verify my assembly using the `pa` print assembled command, and then write the intruction using the `wa` write assembled command.
...
Using the information gathered so far, I created a file containing these values in `/tmp/masterkey.in` and ran the binary again. This produced similar output, however, I could see a new message `[i] Procesing input code...` as seen below.
...
Finally, after all of this the hash is then converted to upper case and compared against a value in an array. If the value matches, a success message is printed and the loop continues onto the next pin. If not, an error message is printed.
...
So at this point I have the hashes, I know how the final hashes generated and salted but I don't know anything about the pin values themselves. After digging further into the binary I didn't find any further clues and started doing some guess work. 
...
Now that this worked, I decided to try and script this to generate the hashes and compare them against the expected hashes from the binary. The working script is available [here](https://gist.github.com/arbitraryrw/0e500bf82b48e85f35393fd17c72190c).
...
The precomputed pin key spells out `chaosMoNKeys`. The last step is to copy and paste the pre-computed pins it into the `/tmp/masterkey.in` directory.
...
# Conclusion 
Cyberlock is a really interesting and creative CTF challenge. The mechanic of making the flag depend on twelve pins is really smart, it forces the attacker to think - I really enjoyed that. Also, I appreciated the attempt to rate limit attackers. It provided a nice entry level problem to teach players how to use r2 practically.

A big thank you to the Radare2 team for organising the conference and to [@hexploitable](https://twitter.com/hexploitable) for writing the challenge. I got in contact with [@hexploitable](https://twitter.com/hexploitable) and they were kind enough to share the challenge source [here](https://gist.github.com/hexploitable/25e1805b832fa3e2c25ba4875ffce2dd).
